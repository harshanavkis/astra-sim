...
- Smoke (`smoke.csv`, 4-NPU 1 MB all-to-all): b3_ideal_rdma 10195 < loom 14705 < b1_gpu_rdma 20105 < b2_cpu_proxy 36744. Loom vs B1 **+26.86%**.
...
| cross_rack | 64 KB | +37.8% | +69.9% |
| cross_rack | 256 KB | +24.7% | +55.6% |
| cross_rack | 1 MB | +10.4% | +30.6% |
| cross_rack | 4 MB | +3.1% | +10.9% |
...
- Matrix (`matrix.csv`, 24 Loom-vs-B1 cells): +3.4...+39.5% @1 MB, +0.2...+22.8% @16 MB, -0.3...+4.3% @64 MB.
...
- Apps (`apps.csv`): gpt3_dense 32 ranks **+5.35%**; gpt3_dense 64 ranks **+4.97%**; mixtral_moe 16 ranks **+7.52%**; mixtral_moe 64 ranks **+15.04%** vs B1.
...
|---|---|---|---|---|---|
| gpt3_dense | 32 | 80.4% | +3.2% | +14.3% | **+5.35%** |
| gpt3_dense | 64 | 88.3% | +3.8% | +14.1% | **+4.97%** |
| mixtral_moe | 16 | 62.8% | +4.0% | +13.4% | **+7.52%** |
| mixtral_moe | 64 | 82.3% | +15.4% | +13.2% | **+15.04%** |
...
**OPEN: `t_pipe_local` appears to double-count forwarding.**
`gen_network_config.py` comments that "the Loom ToR IS the rack switch
(whose forwarding is already inside fabric_latency); Loom adds only the
table lookups" - but then sets `pipe_local = t_lookup 25 + t_translate 15
+ t_forward 10 = 50`, charging forwarding a second time. By its own
rationale it should be **40 ns**. This is the whole in-rack penalty (M2
in-rack is -9.4% at 4 KB) and one of the two surviving negative matrix
cells, so it is worth settling before T3 reports per-stage numbers and
bakes the double-count in. A positive adder IS justified - Loom's ToR does
range-match + bounds + translate that a plain NVSwitch does not - but 50
looks like 40.
