...
def scale_lines():
    ...
    out = ["- Scale sweep (`scale_sweep.csv`, A5) - 8 XPUs/rack fixed, RACK "
           "COUNT is the axis. Loom's benefit is a function of cluster width, "
           "so no single number is meaningful without stating the scale:", "",
           "| GPUs | " + " | ".join(f"{c} {s} MB" for c, s in combos) + " |",
           "|---" * (len(combos) + 1) + "|"]
    for g in gpus:
        cells = []
        for c, s in combos:
    ...
    out += ["", "  Mechanism: a hierarchical collective spreads a roughly "
                "constant dim1 byte count over 2(r-1) steps, so per-step bytes "
                "fall as ~1/r while per-step LATENCY is fixed. Few racks = few "
                "fat steps = bandwidth-bound, hiding Loom's 1615-vs-3000 ns "
                "dim1 edge; many racks = many thin steps = latency-bound, where "
                "Loom wins. The exact +0.00% entries at small rack counts are "
                "that hiding, not parity.", ""]
    return out
...
def domain_lines():
    ...
    d = collections.defaultdict(dict)
    for r in rows:
        d[(int(r["xpus_per_rack"]), int(r["racks"]), r["collective"],
           r["size_mb"])][r["system"]] = int(r["wall_cycles"])
    doms = sorted({(k[0], k[1]) for k in d})
    combos = sorted({(k[2], k[3]) for k in d}, key=lambda t: (t[0], int(t[1])))
    total = int(rows[0]["gpus"])
    out = [f"- Scale-up DOMAIN sweep (`domain_sweep.csv`) - STANDALONE (slow: "
           f"{total} GPUs per point). **This experiment runs AGAINST Loom and "
           f"must not be dropped.** Cluster size is held at {total} GPUs while "
           f"the scale-up domain grows (HGX 8 -> NVL36 -> NVL72), so racks "
           f"shrink and more traffic stays in-rack - traffic Loom does not "
           f"change:", "",
           "| GPUs/rack | racks | " + " | ".join(f"{c} {s} MB" for c, s in combos) + " |",
           "|---" * (len(combos) + 2) + "|"]
    for m, r in doms:
        cells = []
        for c, s in combos:
    ...
    out += ["", "  Bounds which argument carries the paper in which "
                ...
                "deployments are correspondingly LARGER, so rack count - and "
                "with it the cross-domain traffic fraction - recovers. That "
                "defence is NOT yet demonstrated: simulating 72-GPU racks "
                "beyond ~576 GPUs did not complete here, so it needs the "
                "validated analytical proxy (catalog B7/A5).", ""]
    return out
...
def sm_lines():
    ...
    out = ["- SM-reservation sweep (`sm_sweep.csv`, A6) - STANDALONE. There is "
           "no SM in ASTRA-sim: the tax is a roofline derating, "
           "`peak_perf = 989 * (132-k)/132` on the BASELINE only, since "
           "nothing runs on Loom's accelerators. k=20 is DeepSeek-V3's "
           "disclosed reservation. **k=0 isolates the pure communication "
           "benefit**, which is the most useful row here:", "",
    ...
    for app in apps:
        for k in ks:
            ...
            lw, le = v["loom"]
            bw, be = v["b1_gpu_rdma"]
            out.append(f"| {app} | {k} | {gain(bw, lw):+.2f}% | "
                       f"{gain(be, le):+.2f}% | "
                       f"{gain(bw - be, lw - le):+.2f}% |")
    out += ["", "  The k=0 rows separate the two claims cleanly: for the dense "
                "LLM the communication benefit is essentially ZERO (+0.02% "
                "wall), so its entire gain is SM reclamation; for MoE the "
                "communication benefit carries it (+13.90% at k=0) and "
                "reclamation adds ~1.4pp. Quote them separately rather than "
                "reporting one blended number.",
            "  Model limitation, measured: `perf` is a `min()`, so only "
                "COMPUTE-BOUND nodes are derated and memory-bound ones escape "
                "the tax - the compute gain lands at +13.2/+14.1% instead of "
                "the ideal 20/132 = 15.15%. Deriving `local-mem-bw` by the same "
                "factor (`MEMBW=1`) recovers exactly 15.15% for both apps and "
                "moves wall to +15.68% / +5.54%. So the shipped model "
                "UNDER-states the SM tax, i.e. errs against Loom.", ""]
    return out
...
def build():
    ...
    for fn in (smoke_lines, p2p_lines, credits_lines, scale_lines,
               domain_lines, sm_lines, tpipe_lines, regime_lines):
        out += fn()
    out += matrix_lines()
    out += apps_lines()
