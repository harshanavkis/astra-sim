...
> Last updated: 2026-07-08 (through commit `be83626`).
...
| dim1 latency = wire (600 ns, cut-through ToR class) + `roce_stack` (700 ns, NIC-class) + 2·`t_pipe` (500 ns*) | cross-rack peer store: source ToR pipeline (validate/match/encap) → RoCE → destination ToR pipeline (decap/bounds/translate) |
